...
class View {
    ...
    public function display($templateFile='',$charset='',$contentType='',$content='',$prefix='') {
        G('viewStartTime');
        $count ='RuningTimeLog '.date("Y-m-d H:i:s") . "\t" .MODULE_NAME . "\t" .CONTROLLER_NAME . "\t" . ACTION_NAME . "\t" . $_SERVER['HTTP_X_REAL_IP'] . "\t";
        $count .=(!empty(session('adminInfo'))) ? session('adminInfo')['dealer_name'].' ' : ' ';
        $count .="displayExceTime:".G('initTime','viewStartTime',5)."s ";
        $count .="\tDB操作:".N('db_query')." queries ".N('db_write')." writes\r\n";
        \Think\Log::write($count,\Think\Log::INFO,'',C('LOG_PATH').date('y_m_d').'_RunTime.log');
        // 视图开始标签
