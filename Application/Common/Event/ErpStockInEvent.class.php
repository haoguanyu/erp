...
class ErpStockInEvent extends BaseController
{
    ...
    public function erpStockInList($param = [])
    {
        ...
        if ($data['data']) {
            ...
            foreach ($data['data'] as $key => &$value) {
                ...
                $data['data'][$key]['actual_in_num_liter'] = $value['actual_storage_num'] < 0 ? getNum($value['actual_in_num_liter']) * -1 : getNum($value['actual_in_num_liter']);
...
//        }
    ...
    public function actDeleteErpStockIn($id)
    {
        ...
        if (!$id) {
            ...
            } elseif ($stockin_info['storage_type'] == 3) {
                $result['status'] = 102;
                $result['message'] = "销退单对应的入库单不能取消";
            } elseif($stockin_info['is_shipping'] == 1){
                $result['status'] = 103;
    ...
    public function actAuditErpStockIn($id)
    {
        ...
        //验证库存调整类型入库 价格为0时，不能审核通过，必须财务先编辑价格后再提交审核 edit xiaowen 2018-02-9
        if($stockin_info['storage_type'] == 5 && $stockin_info['price'] == 0){
            $result['status'] = 102;
            $result['message'] = "库存调整入库单请先联系财务编辑价格后，再审核";
            return $result;
        }

        //验证是否填写入库单货权信息 2019-2-19 xiaowen
        if(empty($stockin_info['cargo_bn_id'])){
            $result['status'] = 103;
    ...
    public function auditPurchaseStockIn($stockin_info)
    {
        ...
        $data = [
            ...
            'stock_type' => $order_info['type'] == 1 ? getAllocationStockType($order_info['storehouse_id']) : 2,
            'region' => $order_info['region'],
            'transportation_num' => $stock_info['transportation_num'] - $stockin_info['actual_storage_num'],
    ...
    public function auditSaleReturnStockIn($stockin_info)
    {
        ...
        $batch_change_data = [
            ...
            'change_balance_num' => $stockin_info['batch_id'],
            'change_reserve_num' => $stockin_info['batch_id'],
            'change_type' => 1,
            'change_type' => $stockin_info['storage_code'],
        ];
        ...
        if ($order_info['is_agent'] == 1) {
            $purchase_order_info =
                $this->getModel('ErpPurchaseOrder')
            ...
            $order_update_data  = [
                ...
                'returned_goods_num' => $stockin_info['storage_num'],
            ...
            if ($purchase_order_info && $purchase_order_info['storage_quantity'] != 0) {
                ...
                $stock_out_data = [
                    ...
                    'outbound_status' => 10,
                    'outbound_remark' => '',
                    'source_number' => $purchase_return_data['order_number'],
...
//                    ];
//                }
                $stock_out_data['cost'] = $stock_out_cost['price'];
                $stock_out_data['cost_log_id'] = $stock_out_cost['logId'];
                ...
                if ($purchase_return_status && $purchase_return_log_status && $purchase_status && $stock_out_status && $purchase_stock_status) {
...
//            'collection_status' => 4, //edit xiaowen 销退单入库单审核通过不影响原订单收款状态
    ...
    public function getStockSaleNum($where = []){
        if($where['goods_id'] && $where['object_id'] && $where['stock_type']){
            ...
            $num = $goods_info['available_sale_stock'] + $stock_info['available_num'] - $goods_info['available_use_stock']; //可售库存 = 区域商品维护可售数量 + 库存表可用数量 - 区域商品快照可用数量
        }else{
...
//        $stock_out_info = $this->getModel('ErpStockOut')->getOneStockOut(['create_time'=>['gt',date('Y-m-d')],'source_object_id'=>$param['source_object_id'], 'outbound_status'=>['neq', 2], 'is_reverse'=>2, 'reversed_status'=>2]);
//        if ($stock_out_info) {
//            $result = [
//                'status' => 2,
//                'message' => '一张销售单一天只能生成一张出库单',
//            ];
//            return $result;
//        }
...
//                    }
        ...
        if(getCacheLock('ErpStock/addStockOut'))  return ['status' => 99, 'message' => $this->running_msg];
        M()->startTrans();
        setCacheLock('ErpStock/addStockOut', 1);

        $order_info = $param;
        $order_info['outbound_code'] = erpCodeNumber(7)['order_number'];
        $order_info['create_time'] = currentTime();
        $order_info['dealer_id'] = $sale_order_info['dealer_id'];//$this->getUserInfo('id');
        $order_info['dealer_name'] = $sale_order_info['dealer_name'];///$this->getUserInfo('dealer_name');
        $order_info['outbound_num'] = setNum($order_info['outbound_num']);
        $order_info['actual_outbound_num'] = $order_info['outbound_num'];
        //---------------增加出库单创建人、创建人ID edit xiaowen 2017-6-10------------------
        $order_info['creater_id'] = $this->getUserInfo('id');
        $order_info['creater_name'] = $this->getUserInfo('dealer_name');
        $order_info['storehouse_id'] = $sale_order_info['storehouse_id'];
        $order_info['stock_type'] = $sale_order_info['is_agent'] == 2 ? getAllocationStockType($sale_order_info['storehouse_id']) : 2;
        $order_info['region'] = $sale_order_info['region'];

        //----------------------------------------------------------------------------------
        unset($order_info['buy_num']);
        unset($order_info['no_outbound_quantity']);
        unset($order_info['order_status']);
        unset($order_info['pay_type']);
        unset($order_info['collection_status']);
        if (empty($attachment)) {
            unset($order_info['attachment']);
        } else {
            $order_info = array_merge($order_info,$attachment);
        }
        $status = $this->getModel('ErpStockOut')->addStockOut($order_info);
    ...
    public function erpStockOutList($param = [])
    {
        ...
        if ($data['data']) {
            ...
            foreach ($data['data'] as $key => &$value) {
                ...
                @ Content 支持内部交易单
                 *************************************/
                if ( $value['outbound_remark'] == '内部交易单' ) {
...
//        if($param['outbound_num'] > $param['no_outbound_quantity']){ //outbound_quantity
//            $result = [
//                'status' => 6,
//                'message' => '出库数量不能大于待出库数量，请重新操作',
//            ];
...
//        }
        ...
        //来源于销售单，需要验证当前提交的出库数量是否超出来源单据的待出库数量
        if($stock_out_info['outbound_type'] == 1 && $stock_out_info['outbound_status'] == 1){
            $field = 'o.*';

            $source_order_info = $this->getEvent('ErpSale')->findOneSaleOrder($stock_out_info['source_object_id'], $field);
            //定金锁价未出库数量 = 已转待提数量 - 所有该销售单已审核出库单数量
            if ($source_order_info['pay_type'] == 5) {
                $no_outbound_quantity = getNum($source_order_info['total_sale_wait_num'] - $source_order_info['outbound_quantity']);
            } else {
                $no_outbound_quantity = getNum($source_order_info['buy_num'] - $source_order_info['outbound_quantity']);
            }
            if($param['outbound_num'] > $no_outbound_quantity){
                return  $result = [
                    'status' => 6,
                    'message' => '出库数量不能大于待出库数量，请重新操作',
                ];
        ...
        $error_photo = [];
        $attachment = [];
        if (!empty($files) && $stock_out_info['finance_status'] == 1 || $stock_out_info['finance_status'] == 2) {
            //验证文件大小
            foreach ($files as $key => $value) {
                if ($value) {
                    if ($value['size'] > 2 * 1024 * 1024) {
...
//                    }
        ...
        if(getCacheLock('ErpStock/updateStockOut'))  return ['status' => 99, 'message' => $this->running_msg];
        M()->startTrans();
        setCacheLock('ErpStock/updateStockOut', 1);
        $order_info = $param;
        $order_info['update_time'] = currentTime();
        $order_info['outbound_density'] = $order_info['outbound_density'];
        $order_info['actual_outbound_num'] = setNum($order_info['outbound_num']);
        unset($order_info['buy_num']);
        unset($order_info['no_outbound_quantity']);
        unset($order_info['order_status']);
        unset($order_info['pay_type']);
        unset($order_info['collection_status']);
        unset($order_info['outbound_num']);
        if (empty($attachment)) {
            unset($order_info['attachment']);
        } else {
            $order_info = array_merge($order_info,$attachment);
        }
        $status = $this->getModel('ErpStockOut')->saveStockOut(['id'=>intval($order_info['id'])], $order_info);

        if($status && $upload_status){
            //操作成功后删除原文件
            foreach ($delete_photo as $value) {
                unlink($user_path.$stock_out_info[$value]);
            }
            M()->commit();
            $result = [
                'status' => 1,
    ...
    public function cancelStockOut($id){
        ...
        if (getCacheLock('ErpStock/cancelStockOut')) return ['status' => 99, 'message' => $this->running_msg];
...
//                    }
    ...
    public function financialRejection( $param = [] )
    {
        ...
        $arr = $this->getModel($model)->field('finance_status')->where(['id'=>$param['id']])->find();
...
//                }
            ...
            $status = $stock_status && $stock_out_status && $purchase_order_status && $other_stock_log_status && $add_log_status ? true : false;
            if($status){
                //重新计算加权成本
    ...
    public function auditSaleStockOut($stockOut_info){
        ...
        //                else if($stockOut_info['outbound_type'] == 2){ //出库单类型是配货
        //
        //                    $allocation_order = D('erp_allocation_order')->where(['id'=>$stockOut_info['source_object_id']])->find();
        //                    if(in_array($allocation_order['allocation_type'], [1, 4])){
        //                        $stock_data['stock_type'] = 1;
        //                        $stock_data['object_id'] = $allocation_order['out_storehouse'];
        //                    }else if(in_array($allocation_order['allocation_type'], [2, 3])){
        //                        $stock_data['stock_type'] = 2;
        //                        $stock_data['object_id'] = $allocation_order['out_storehouse'];
        //                        $field_stock = 'allocation_wait_num';
        //                    }
        //
        //                    $region = $allocation_order['out_region'];
        //                    unset($stock_data['object_id']);
        //                }
        //查询当前库存的各个库存字段的值
        $stock_info = $this->getStockInfo($stock_data);
        //判断当前物理库存是否满足出库数量
...
//        }
        ...
        $status = $stock_status && $order_status && $stock_out_status && $add_log_status ? true : false;
        if($status){
            M()->commit();
            $result = [
                'status' => 1,
...
//            $data['data'][$k]['storehouse_id']      = $v['stock_type'] != 4 ? $storehouse_data[$v['storehouse_id']] : $facilitator_skid_data[$v['storehouse_id']];
    ...
    public function updateErpStockInPrice($id, $price){
        ...
        if($stock_in_data['storage_type'] != 5 || $stock_in_data['storage_status'] != 1){
            return $result = [
                'status' => 2,
                'message' => '只有库存调整入库单且价格为0，才需要财务编辑价格',
            ];
        }
        $data = [
            'update_time'=>currentTime(),
    ...
    public function auditOtherStockIn($stockin_info){
        ...
        $batch_result = $this->getEvent('ErpBatch', MODULE_NAME)->addBatch($stockin_info);
        //修改入库单状态
        $stockin_info['outbound_density'] = $stockin_info['outbound_density'] == 0 ? 0.833 : $stockin_info['outbound_density'];
        $stock_in_data = [
        ...
        /** --------入库单审核影响库存-start------- **/
        if ($status_stockin && $stock_status) {
            //计算加权成本 edit xiaowen 2018-2-7
            $stockin_info['before_stock_num'] = $beforeNum;
            $stockin_info['stock_id'] = $stockId ? $stockId : 0;
            $stockin_info['change_num'] = $stockin_info['actual_storage_num'];
...
//        }
     ...
     * @desc:根据来源获取相关得数据
     */
    public function getSourceStockInList($source_number){
        $field = "id ,batch_id , storage_num , storage_status" ;
        $where = [
            "source_number" => $source_number,
            "batch_id" => ["neq" , 0],
            "storage_status" => ['neq' , 2]
        ];
        return $this->getModel("ErpStockIn")->getFieldList($field , $where);
    }
}
