...
//            if(!$this->isNewCustomer($param['user_id']) && setNum($param['buy_num']) > $stockSaleNum && $storehouse_type == 1){ //新客户也预留，date:2017/05/25
            ...
            $status = $id = $this->getModel('ErpSaleOrder')->addSaleOrder($data);

            $log_data = [
                'sale_order_id' => $id,
                'sale_order_number' => $data['order_number'],
                'log_type' => 1,
                'log_info' => serialize($data),
            ];
            $log_status = $this->addSaleOrderLog($log_data);
           //=========================根据用户是否为老用户，判断是否增加库存的销售预留=========================
//            $is_new_customer = $this->isNewCustomer(intval($param['user_id']));
//            if(!$is_new_customer || $param['pay_type'] == 5){ //新客户也增加预留，date:2017/05/25

            $order_info = $this->findOneSaleOrder($id, 'o.is_agent, o.id, o.order_number, o.goods_id, o.storehouse_id,o.buy_num, o.region, o.pay_type, o.order_status, o.collection_status');
//            print_r($order_info);
//            exit();
            ...
            $stock_where = [
                ...
                'stock_type' => $order_info['is_agent'] == 2 ? getAllocationStockType($order_info['storehouse_id']) : 2,
            ...
            $stock_info['sale_reserve_num'] = $data['sale_reserve_num']; //重置最新的预留库存
            //------------------计算出新的可用库存----------------------------
    ...
    /**
     * 通用新增销售单方法
     * 本方法不做生成销售单的数据检验，请在调用本方法前做好检验及数据组装
     * @author xiaowen
     * @time 2019-1-24
     * @param $data 生成销售单的数据，必须在调用本方法前组装好数据
     * @return  boolean
     */
    public function CommonAddSaleOrder($data){
        $addOrderData = $data ;
        $order_status = $id = $this->getModel('ErpSaleOrder')->addSaleOrder($data);
        ...
        $data['available_num'] = $this->getEvent('ErpStock')->calculateAvailableNum($stock_info);
        //----------------------------------------------------------------
        $stock_log_type = 1; //新增动作影响库存
        $orders = [
            'object_number' => $order_info['order_number'],
            'object_type' => 1,
            'log_type' => $stock_log_type,
            "our_company_id" => (session('erp_company_id')) ? session('erp_company_id') : $addOrderData['our_company_id']
        ];
        //----------------更新库存，并保存库存日志-------------------------
        $data['our_company_id'] = $addOrderData['our_company_id'] ;
        $stock_status = $this->getEvent('ErpStock')->saveStockInfo($data, $order_info['buy_num'], $orders);
        //======================================================================================================
        return $order_status && $log_status && $stock_status ? true : false;
    }
...
//                }
                ...
                if(checkInErpCompany($order_info['company_name']) || $order_info['order_source'] == 3){ //内部账套或app(老吕找油)销售单 不走审批流 edit xiaowen 2019-5-21
...
//            }
            ...
            $saleOrderInfo['company_name'] = $this->getModel('ErpCustomer')->where(['id'=>$saleOrderInfo['company_id']])->getField('customer_name');
            if(checkInErpCompany($saleOrderInfo['company_name'])  || $saleOrderInfo['order_source'] == 3){ //内部账套或app(老吕找油)销售单 不走审批流 edit xiaowen 2019-5-21
    ...
    public function entryLoss($param){
        ...
        if(!isset($param['lossRatio']) || empty($param['lossRatio'])){
            return  $result = [
                'status' =>11,
                'message' => '请选择损耗比例',
            ];
        }
        $saleOrderInfo = $this->getModel('ErpSaleOrder')->findSaleOrder(['id'=>intval($param['orderId'])]);
        ...
        if($saleOrderInfo['loss_num']){
            return $result = [
                'status' => 10,
                'message' => '已经录入了损耗,请到损耗列表查询',
            ];
        }
        if ($saleOrderInfo['entered_loss_amount'] > 0) {
            return $result = [
                'status' => 4,
        ...
        //获取出库单数据信息
        $whereStockOut = [
            "id" => ["in" ,array_keys($param['stockOut'])]
        ];
        $stockOutList = $this->getEvent('ErpStockOut')->getStockOutLists("" , $whereStockOut) ;
        if(count($param['stockOut']) != count($stockOutList)){
            return $result = [
                'status' => 5,
                'message' => '出库单数据缺失，请核查',
            ];
...
//        $field = "b.id  , c.id as cargoId" ;
//        $whereBatch = ["b.id" => ['in' , $batchIds]] ;
//        $batchCargo = $this->getEvent("ErpBatch")->getBatchCargo($field , $whereBatch);
        $totalLossNum = 0 ;
        $lossOrder = [] ;
        $totalReasonableLoss = 0 ;
        foreach ($stockOutList as $value){
            if(($value['outbound_status'] != 10) || ($value['outbound_type'] != 1)
                || ($value['is_reverse']!= 2) || ($value['reversed_status']!= 2)){
                return $result = [
                    'status' => 6,
                    'message' => '出库单'.$value['order_number']."状态信息不正确",
                ];
            }
            $actual_outbound_num = getNum($value['actual_outbound_num']) ;
            if($actual_outbound_num < 0.1){
                return $result = [
                    'status' => 13,
                    'message' => '出库单低于0.1吨的不可输入损耗信息',
                ];
            }
            $lossNum = setNum($param['stockOut'][$value['id']]['lossNum']) ;
            if($value['actual_outbound_num'] <=  $lossNum){
                return $result = [
                    'status' => 12,
                    'message' => '损耗数量，不能大于、等于出库数量',
                ];
            }

            if(empty($lossNum) || $lossNum < 0){
                return $result = [
                    'status' => 9,
                    'message' => "损耗数量不正确",
                ];
            }
            $reasonableLoss =  setNum($param['stockOut'][$value['id']]['reasonableLossNum']) ;
            $overNum = setNum($param['stockOut'][$value['id']]['overLossNum']) ;
            if(($reasonableLoss + $overNum) != $lossNum){
                return $result = [
                    'status' => 14,
                    'message' => "合理损耗加上超损数量不能超过出库数量",
                ];
            }
            ...
            $lossOrder[] = [
                ...
                "type" => 3 ,
                "source_number" => $value['outbound_code'] ,
                "loss_ratio" =>  $param['lossRatio'],
//                "batch_id" => $value['batch_id'] ,
//                "cargo_bn_id" => $batchCargo[$value['batch_id']] ,
                "our_company_id" => $value['our_company_id'] ,
                "company_id" => $saleOrderInfo['company_id'] ,
                ...
                "loss_num" => $lossNum ,
                "reasonable_loss_num" => $reasonableLoss ,
                "exceed_loss_num" => $overNum ,
                "price" => $saleOrderInfo['price'] ,
                "creater" => $_SESSION['erp_']['erp_adminInfo']['id'] ,
                "create_time" => currentTime()
            ];
            $totalLossNum += $lossNum ;
            $totalReasonableLoss += $reasonableLoss;
        }
        $updateSaleOrder = [
            'is_loss' => 1,
            'loss_num' => $totalLossNum,
            'update_time' => currentTime(),
        ];
        $logData = [
            'sale_order_id' => $saleOrderInfo['id'],
...
//        $reasonableLoss = $saleOrderInfo['outbound_quantity'] * $param['lossRatio'] / 1000 ;
//        if($reasonableLoss < $totalReasonableLoss){
//            return $result = [
//                'status' => 15,
//                'message' => "合理损耗不能超过最大有效合理损耗值",
//            ];
//        }
        if (getCacheLock('ErpSale/entryLoss'))
            return ['status' => 99, 'message' => $this->running_msg];
        setCacheLock('ErpSale/entryLoss', 1);
        M()->startTrans();
        //损耗单
        if(!$this->getModel("ErpLossOrder")->addAll($lossOrder)){
        ...
        //销售单
        if(!$this->getModel('ErpSaleOrder')->saveSaleOrder(['id'=>intval($param['orderId'])], $updateSaleOrder)){
            cancelCacheLock('ErpSale/entryLoss');
            M()->rollback() ;
            return $result = [
                'status' => 7,
        ...
        //销售单日志
        if(!$this->addSaleOrderLog($logData)){
            cancelCacheLock('ErpSale/entryLoss');
            M()->rollback() ;
            return $result = [
                'status' => 8,
    ...
    public function createWorkflowNew($sale_order_info = []){
        ...
        if($sale_order_info){
            ...
            }else if($sale_order_info['business_type'] == 5){    //ids类型，走ids审批流 2019-5-8 awen
                $workflow_step = IdsWorkflow(3);
            }
            //print_r($workflow_step);
            $workflow_data = [
                'workflow_type' => 1,
                'workflow_order_number' => $sale_order_info['order_number'],
                'workflow_order_id' => $sale_order_info['id'],
     ...
     * @desc:根据出库单，修改红冲数量
     */
    public function saveLoss($outCode , $lossNum){
        //查询销售单号
        $whereStock = [
            "outbound_code" => $outCode,
            "outbound_type" => 1 ,
            "outbound_status" => 10 ,
            "reversed_status" => 2 ,
            "is_reverse" => 2
        ...
        $field = "source_number" ;
        $saleOrderNumber = $this->getEvent("ErpStockOut")->getStockOutLists($field , $whereStock);
        if(!$saleOrderNumber){
            return ['status' => 16  , "message" => "为获取到出库单相关数据"] ;
        }
        $whereSale = [
            "order_number" => $saleOrderNumber ,
            "is_void" => 2 ,
        ] ;
        $field = "id , is_loss , loss_num , entered_loss_amount" ;
        $saleInfo = $this->getModel("ErpSaleOrder")->findSaleOrder($whereSale , $field);
        if(!$saleInfo){
            return ['status' => 17  , "message" => "为获取到销售单相关数据"] ;
        }
        if($saleInfo['entered_loss_amount'] > 0){
            return ['status' => 19  , "message" => "损耗已处理，请先到应收管理进行处理"] ;
        }
        $updateLossNum = $saleInfo['loss_num'] - $lossNum ;
        if($updateLossNum < 0){
            return ['status' => 20  , "message" => "请确认可回退数量"] ;
        }
        $updateSale = [
            "is_loss" => ($updateLossNum == 0) ? 2 : $saleInfo['is_loss'] ,
            "loss_num" => $updateLossNum
        ];
        if(!$this->getModel("ErpSaleOrder")->saveSaleOrder(["id" => $saleInfo['id']] , $updateSale)){
            return ['status' => 18  , "message" => "更新销售单数据失败，请重新尝试"] ;
        }
        return ['status' =>1 , "message" => "更新销售单数据成功"] ;
    }
}
