 ...
 * 供货单模型
 */
class ErpSupplyModel extends BaseModel
{
    ...
    {
        return $this->alias('s')
            ->field('sum(sale_num)/'.C('IO_NUM').' as total_sale_num, sum(lock_num)/'.C('IO_NUM').' as total_lock_num')
            ->where($where)
            ->join('oil_erp_goods g on s.goods_id = g.id', 'left')
            ->find();
    }
